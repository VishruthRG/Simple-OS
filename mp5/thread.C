...
static void thread_shutdown() {
    ...
    */
    SYSTEM_SCHEDULER->terminate(Thread::CurrentThread()); // terminate the thread
    delete current_thread; // remove the current_thread
    SYSTEM_SCHEDULER->yield(); // yield cpu to next thread in the ready queue
}
...
static void thread_start() {
     ...
     Machine::enable_interrupts(); // enable interrupts
}
